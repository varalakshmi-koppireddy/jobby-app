...
class AboutJobItem extends Component {
  ...
  getJobData = async () => {
    ...
    if (responseJobData.ok === true) {
      const fetchedDataJobData = await responseJobData.json()
      const updatedDataJobData = [fetchedDataJobData.job_details].map(
        eachItem => ({
      ...
      const updatedSimilarJobDetails = fetchedDataJobData.similar_jobs.map(
        eachItem => ({
          companyLogoUrl: eachItem.company_logo_url,
          employmentType: eachItem.employment_type,
          id: eachItem.id,
      ...
      this.setState({
        jobsDataDetails: updatedDataJobData,
